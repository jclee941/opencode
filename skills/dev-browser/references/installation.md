...
go run ./server.go &
# Or for headless:
go run ./server.go --headless &
```

**Windows (PowerShell):**
```powershell
cd "$env:USERPROFILE\.config\opencode\skills\dev-browser"
Start-Process -NoNewWindow -FilePath "go" -ArgumentList "run", ".\\server.go"
# Or for headless:
Start-Process -NoNewWindow -FilePath "go" -ArgumentList "run", ".\\server.go", "--headless"
```

**Windows (CMD):**
```cmd
cd %USERPROFILE%\.config\opencode\skills\dev-browser
start /B go run .\server.go
```
